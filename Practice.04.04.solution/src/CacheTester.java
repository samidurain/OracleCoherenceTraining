...
public class CacheTester {
	...
	public static void main(String[] args) {
		...
		System.out.println("Timing reads from the cache:");
		for (Iterator<Long> i = customers.keySet().iterator(); i.hasNext();) {
			System.out.println("==================================================");

			Long key = i.next();
			for (int readCount = 0; readCount < 5; readCount++) {
				stopWatch.start();
				Customer customer = (Customer)customers.get(key);
				stopWatch.stop();

				System.out.println("Customer: ID=" + customer.getId() + " Name="
						+ customer.getName() + " Read time=" + stopWatch.toString());
			}
		}
